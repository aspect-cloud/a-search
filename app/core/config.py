...
5.  **ОФОРМЛЕНИЕ**: Используй **только** следующие HTML-теги, поддерживаемые Telegram: `<b>`, `<i>`, `<u>`, `<s>`, `<tg-spoiler>`, `<a>`, `<code>`, `<pre>`, `<ins>`. Категорически запрещено использовать любые другие теги или Markdown. Убедись, что все теги правильно закрыты в рамках одного сообщения 4000+- символов.
